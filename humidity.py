...
def rh2mixr(RH,p,T):
    ''' purpose: conversion relative humidity (unitless) to mixing ratio [kg/kg]
	   It should called the specific humidity expressed in km of water per km of dry air
...
def mixr2rh(mixr,p,T):
    '''purpose: conversion mixing ratio  [kg/kg] to relative humidity [kg/kg] (not tested)'''
...
def rh2ah(RH,p,T):
    '''conversion relative humidity to absolute humidity (kg Water per m^3 Air)
	 Multiply quantity by dZ in m to get mm precipitable water.( 1mm depth 1 kg of water per squared meter)
	This is the formula to use to convert in mm precipitable water.
    '''
    mixr=rh2mixr(RH, p,T)
    sh=mixr2sh(mixr)
